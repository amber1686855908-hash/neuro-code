...
定义只读技能文件发现的领域模型. 技能文件由仓库提供,不执行、不从网络加载,也不能冒充 system 或 user 消息;发现过程确定性、有界且失败关闭.
"""
...
class SkillScope(IntEnum):
    ...
    定义技能发现位置对应的范围和优先级. 数值越小优先级越高,即 Local 覆盖 Repo,Repo 覆盖 User.
    """
...
def normalize_skill_name(name: str) -> str:
    ...
    将技能名称规范化为 slug. 非法字符替换为连字符,连续连字符合并,并去掉首尾连字符.
    """
    result: list[str] = []
...
class ParsedFrontmatter:
    ...
    表示从 SKILL.md 文件解析 YAML frontmatter 的结果."""
...
def parse_frontmatter(
    ...
    从 SKILL.md *content* 解析 YAML frontmatter. 使用有界的逐行解析器,格式错误时保留已成功解析的字段并使用回退名称.
    """
...
def extract_skill_body(content: str) -> str:
    ...
    提取技能文件正文,即 YAML frontmatter 之后的全部内容. 只移除 frontmatter,不负责解析其中字段.
    """
    stripped = content.lstrip()
    match = _FRONTMATTER_RE.match(stripped)
    if match is None:
...
def _parse_supported_index(raw_index: str, upper_bound: int) -> int | None:
    ...
    返回安全的位置索引,并保留类似价格的 token 不变. 有界窗口避免将攻击者控制的大整数解释为参数.
    """
    if len(raw_index) > 6:
        return None
    index = int(raw_index)
...
def apply_skill_substitutions(
    ...
    **Suffix rule**:
    ...
    对技能正文执行变量替换. 支持 `$ARGUMENTS[N]`、`$N`、`$ARGUMENTS` 和 `${SKILL_DIR}`,并按规则追加参数后缀.
    """
...
class SkillDiscoveryResult:
    ...
    def model_context_text(self) -> str:
        ...
        将发现的技能渲染为供模型使用的紧凑列表. 只展示名称、描述和使用时机,正文需通过只读 skill 工具显式加载.
        """
        if not self.files:
            return ""
        entries: list[str] = []
    ...
    def skill_message(self) -> Message:
        ...
        构建携带技能列表的合成 User 消息. 消息使用 `SyntheticReason.AVAILABLE_SKILLS` 标记,并且只存在于内存中的模型上下文.
        """
