...
from neuro_code.ports.terminal import TerminalPlatform, TerminalPlatformSession
...
class LocalInteractiveTerminalManager:
    ...
    def __init__(
        ...
        *,
        ...
        platform: TerminalPlatform | None = None,
        max_sessions: int = 8,
    ) -> None:
        ...
        self._platform = _default_platform() if platform is None else platform
    ...
    async def create_exec(
        ...
        *,
        ...
        resolved_cwd = resolve_workspace_path(self._workspace, cwd, must_exist=True)
...
def _default_platform() -> TerminalPlatform:
    if os.name == "posix":
        from neuro_code.adapters.posix_pty import PosixPtyPlatform

        return PosixPtyPlatform()
    if os.name == "nt":
        from neuro_code.adapters.windows_pty import WindowsConPtyPlatform

        return WindowsConPtyPlatform()
    raise TerminalError(f"interactive terminals are unsupported on platform {os.name!r}")
