...
class _NativeWindowsJobApi:
    ...
    提供 ``WindowsJobObject`` 使用的 kernel32 调用的小型类型化门面."""
...
class WindowsJobObject:
    ...
    管理已配置的 Win32 Job Object 及分配给它的进程."""
    ...
    def process_creation_handle(self) -> int:
        ...
        为进程创建属性列表借用 Job 句柄. 调用方必须在整个 CreateProcessW 调用期间保持对象存活,且不能关闭借用的数值句柄.
        """
