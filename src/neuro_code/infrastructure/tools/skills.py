...
:func:`neuro_code.infrastructure.workspace.instructions._toctou_safe_read`).
...
:func:`neuro_code.domain.workspace.skills.apply_skill_substitutions` for the full
substitution semantics.

提供技能正文加载工具. 模型先看到紧凑技能列表,需要时再通过此工具读取完整正文.
"""
...
class SkillTool:
    ...
    按名称加载已发现技能的完整正文. 工具只读,执行有界且抗符号链接读取,移除 YAML frontmatter 后应用变量替换并返回 XML 内容块.
    """
    ...
    async def execute(self, arguments: Mapping[str, Any], context: ToolContext) -> ToolResult:
        ...
        def load() -> str:
            ...
            读取技能文件,移除 frontmatter,执行变量替换并格式化结果."""
...
def _find_skill(name: str, skills: tuple[SkillInfo, ...]) -> SkillInfo | None:
    ...
    按精确名称查找技能. 发现阶段已经按名称去重,因此最多只有一个匹配项.
    """
    for skill in skills:
        if skill.name == name:
            return skill
...
def _list_bundled_files(skill_dir: Path) -> list[str]:
    ...
    列出技能目录中最多 ``_MAX_BUNDLED_FILES`` 个文件. 排除 SKILL.md、符号链接、目录、重解析点和含控制字符的名称.
    """
    try:
...
def _format_output(
    ...
    将技能正文格式化为 ``<skill_content>`` XML 块,同时包含技能名称、正文、基础目录和最多 10 个附带文件示例.
    """
    lines: list[str] = []
