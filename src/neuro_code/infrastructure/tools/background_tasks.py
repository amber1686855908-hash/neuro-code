"""Canonical background task tools.

This module owns the bounded background task inspection and kill tools used by
the agent loop: ``task_output``, ``wait_tasks``, and ``kill_task``. All tools
delegate through the ``ToolContext.background_tasks`` port and consume
completion bookkeeping via ``mark_completions_reported``; they do not own the
background task manager, process trees, or SQLite persistence. Shared
argument, snapshot, and rendering helpers live here so the legacy module can
remain a one-way compatibility facade.

定义规范的后台任务工具. 本模块负责 task_output、wait_tasks 和 kill_task,并通过 ToolContext.background_tasks 端口委托执行.
"""
