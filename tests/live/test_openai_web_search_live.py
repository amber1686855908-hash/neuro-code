...
def _profile() -> ProviderProfile:
    ...
    if not _env("OPENAI_API_KEY"):
        pytest.skip("SKIPPED_NO_CREDENTIAL: OPENAI_API_KEY is not configured")
    model = _env("NEURO_CODE_LIVE_OPENAI_SEARCH_MODEL") or "gpt-4.1-mini"
    proxy_mode = _env("NEURO_CODE_LIVE_PROXY_MODE") or "environment"
    return ProviderProfile(
        name="live-openai-web-search",
        protocol="openai-responses",
        dialect="standard",
        model=model,
        base_url=_env("NEURO_CODE_LIVE_OPENAI_SEARCH_BASE_URL") or "https://api.openai.com/v1",
...
@pytest.mark.asyncio
async def test_openai_hosted_web_search_executes_and_returns_sources() -> None:
    profile = _profile()
    ...
    result = await ResponsesHostedWebSearchBackend(profile, factory).search(
        WebSearchRequest(
            "What is the official OpenAI API quickstart URL?",
            max_sources=4,
    ...
    assert any("platform.openai.com" in source.url for source in result.sources)
    ...
    credential = _env("OPENAI_API_KEY")
