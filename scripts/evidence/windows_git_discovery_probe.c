...
static void print_attribute_result(const char *name, const wchar_t *path) {
    DWORD attributes;
    DWORD error;
    ...
    printf("\"%s\":{\"success\":%s,\"error\":%lu,\"attributes\":%lu}",
           name,
           attributes != INVALID_FILE_ATTRIBUTES ? "true" : "false",
           (unsigned long)error,
           (unsigned long)(attributes != INVALID_FILE_ATTRIBUTES ? attributes : 0));
}

static void print_directory_open_result(const char *name, const wchar_t *path) {
    HANDLE handle;
    DWORD error;
...
static BOOL enumerate_directory(const wchar_t *path, const wchar_t *sibling_name,
                                BOOL *sibling_visible, DWORD *error) {
    wchar_t pattern[32768];
    WIN32_FIND_DATAW data;
    HANDLE find;

    *sibling_visible = FALSE;
    _snwprintf_s(pattern, ARRAYSIZE(pattern), _TRUNCATE, L"%s\\*", path);
    SetLastError(ERROR_SUCCESS);
    find = FindFirstFileW(pattern, &data);
    *error = find == INVALID_HANDLE_VALUE ? GetLastError() : ERROR_SUCCESS;
    if (find == INVALID_HANDLE_VALUE) return FALSE;
    do {
        if (sibling_name != NULL && wcscmp(data.cFileName, sibling_name) == 0)
...
static void print_file_read_result(const char *name, const wchar_t *path) {
    HANDLE file;
    DWORD error;
    char byte = 0;
    DWORD read = 0;
    ...
    if (file != INVALID_HANDLE_VALUE) {
        if (!ReadFile(file, &byte, 1, &read, NULL)) error = GetLastError();
        CloseHandle(file);
    }
    printf("\"%s\":{\"success\":%s,\"error\":%lu,\"bytes_read\":%lu}",
           name,
           read > 0 ? "true" : "false",
           (unsigned long)error,
           (unsigned long)read);
...
static int authority_check_mode(
    const wchar_t *parent,
    const wchar_t *known_child,
    const wchar_t *sibling_name,
    const wchar_t *sibling_file,
    const wchar_t *protected_ancestor,
    const wchar_t *host_sentinel,
    const wchar_t *outside_secret,
    const wchar_t *controller_secret,
    const wchar_t *junction_target_file) {
    BOOL sibling_visible = FALSE;
    BOOL ignored_visible = FALSE;
    DWORD parent_error = ERROR_SUCCESS;
    DWORD protected_error = ERROR_SUCCESS;
    BOOL parent_enumerated;
    BOOL protected_enumerated;

    parent_enumerated = enumerate_directory(
        parent, sibling_name, &sibling_visible, &parent_error);
    protected_enumerated = enumerate_directory(
        protected_ancestor, NULL, &ignored_visible, &protected_error);
    putchar('{');
    print_attribute_result("stat_parent", parent);
    putchar(',');
    print_directory_open_result("traverse_known_child", known_child);
    printf(
        ",\"enumerate_parent\":%s,\"enumerate_parent_error\":%lu,"
        "\"sibling_name_visible\":%s,",
        parent_enumerated ? "true" : "false",
        (unsigned long)parent_error,
        sibling_visible ? "true" : "false");
    print_file_read_result("sibling_content_read", sibling_file);
    putchar(',');
    print_attribute_result("protected_ancestor_stat", protected_ancestor);
    printf(
        ",\"protected_ancestor_enumerate\":%s,"
        "\"protected_ancestor_enumerate_error\":%lu,",
        protected_enumerated ? "true" : "false",
        (unsigned long)protected_error);
    print_file_read_result("host_sentinel_read", host_sentinel);
    putchar(',');
    print_file_read_result("outside_secret_read", outside_secret);
    putchar(',');
    print_file_read_result("controller_secret_read", controller_secret);
    putchar(',');
    print_file_read_result("junction_target_read", junction_target_file);
    printf("}\n");
    return 0;
}
...
int wmain(int argc, wchar_t **argv) {
    ...
    if (argc == 11 && wcscmp(argv[1], L"authority-check") == 0)
        return authority_check_mode(
