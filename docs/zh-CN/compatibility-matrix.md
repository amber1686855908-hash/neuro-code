...
| ACP stdio/WebSocket | M4 | partial | 已通过官方 `agent-client-protocol==0.11.0` stdio SDK 实现协议版本协商、客户端能力/info 保存、绑定工作区且稳定的 ACP ID 与持久化按需 ACP-to-SQLite-ID alias、`session/new`、工作区范围且有界 cursor 的 `session/list`、`session/load`、保持顺序且有界的 Text/ResourceLink 提示转换、有界/脱敏的可见历史回放、安全恢复配置、文本/工具/usage 流 update、失败关闭权限请求、同 session 单活动 prompt 与跨 session 并行、静默 cancel notification、不删除历史的 close、创建中/EOF/断连清理，以及 session-owned stdio MCP 工具 server。声明 `loadSession: true` 与 `sessionCapabilities.list = {}` / `close = {}`；由于只实现基线 stdio，不声明可选 MCP HTTP/SSE 能力。list 省略内容/私有/provider 元数据，并使用不暴露内部 ID 的随机连接局部 cursor。非空 `additionalDirectories` 仍会被拒绝。Resume/delete/fork、MCP HTTP/SSE/ACP 传输与非工具 MCP 能力、图片/音频/embedded content 与多媒体回放、客户端 `fs/*`/`terminal/*`、WebSocket 和扩展仍不支持。SDK 0.11 需要打开其 unstable router 门才能暴露 `session/close`，会忽略 malformed JSON frame，也会接受错误的入站 `jsonrpc` 版本再输出规范化 2.0 响应；生产实现不会用自制 ACP 调度器掩盖这些 SDK 行为 |
