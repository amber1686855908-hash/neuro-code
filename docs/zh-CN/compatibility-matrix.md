...
| ACP stdio/WebSocket | M4 | partial | 已通过官方 `agent-client-protocol==0.11.0` stdio SDK 实现协议版本协商、客户端能力/info 保存、绑定工作区且稳定的 ACP ID 与持久化按需 ACP-to-SQLite-ID alias、session create/list/load/resume/fork/delete/prompt/cancel/close、有界/脱敏回放、失败关闭权限、session-owned MCP 工具及有界额外目录。声明 `loadSession: true`、list/delete/fork/resume/close，以及 MCP `http: true`/`sse: true` capability。创建/加载/恢复/分叉接受有界 stdio、Streamable HTTP 和 legacy SSE server；ACP 传输 server 与非工具 MCP 能力仍未支持。启用的 sandbox 仍拒绝额外目录，因为其挂载已固定。客户端 `fs/*`/`terminal/*`、多媒体、WebSocket 与扩展仍待实现。 |
