...
用户提示与助手回答使用克制的不同块，共享同一阅读轴并限制为最多 116 列，不再依靠
`You:` 与 `Assistant:` 日志前缀区分。每条流式回答只在对话流中挂载一次，后续增量直到最终
文本都原地更新，因此完成时不会再从临时区域移动到滚动记录；用户主动向上滚动后也不会被
强制拉回底部。

助手文本使用应用自有的语义配色渲染 Markdown，对标题、强调、代码、列表、链接和表格
进行克制区分。模型文本不会作为 Rich/Textual markup 解释，同时禁用链接点击；用户提示
以及本地或外部值继续按字面文本显示。活动、计划和错误通知使用行内标签与语义层级；模型、
路径和工具不会仅因对象类型获得 accent 色。每个模型步骤会显示客户端观测到首个可行动结果
前的耗时；整轮完成后会在最终回答下方显示总耗时。连续工具调用显示为一个默认折叠的活动组，
摘要保留状态、有界意图或聚合数量、失败与耗时；长 Bash 命令会在摘要中截断。单击活动组，
或聚焦后按 `Enter`/`Space`，可以打开固定高度的 Inline Peek；上/下选择调用，Enter 打开独立
Tool Inspector。Summary 与 Peek 永不加载 artifact 或渲染完整 stdout；Inspector 的 Output/Input/
Meta 与工作区 diff 继续遵守有界、控制字符安全和凭据脱敏边界。敏感、二进制、过大、依赖、
缓存及版本库内部内容保持隐藏；新增/删除行分别使用绿色/红色前景和不同的淡色背景。当前切片
尚不包含 Mermaid 和内嵌媒体。

Composer 下方的一行紧凑无冗余字段名状态投影，左侧显示模型、强度和模式，右侧显示上下文
占用与工作目录；窄布局会省略过长值。永久快捷键栏已移除，可通过 `/help` 或 F1 查看既有
命令参考。等待模型输出时，用户提供的七格折叠脉冲会在待完成助手文案前动画。
...
真实分母。
请求等级与当前实际策略不同时会同时显示两者，例如 `ultracode → max`。状态文案会随界面
语言切换。

输入 `/` 时会显示命令语法和参数提示。候选项包含六档强度、四种模式和当前可选择的供应商 profile
...
使用 `Ctrl+E`、不带参数的 `/effort` 或 `/reasoning` 可以打开六级强度选择器；
