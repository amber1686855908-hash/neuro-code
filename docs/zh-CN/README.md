...
`You:` 与 `Assistant:` 日志前缀区分。每条流式回答只在对话流中挂载一次，后续增量直到最终文本都
...
以及本地或外部值继续按字面文本显示。系统、状态、活动、计划和错误通知使用行内标签与语义
层级；模型、路径和工具不会仅因对象类型获得 accent 色。每个模型步骤会显示客户端观测到
首个可行动结果前的耗时；整轮完成后会在最终回答下方显示总耗时。连续工具调用显示为一个
默认折叠的活动组，摘要保留状态、有界意图或聚合数量、失败与耗时；长 Bash 命令会被截断。
单击活动组，或聚焦后按 `Enter`/`Space`，可以查看既有的有界、控制字符安全、凭据脱敏的
权限、完整命令、输出、输出 artifact 与工作区 diff。敏感、二进制、过大、依赖、缓存及
版本库内部内容保持隐藏；新增/删除行分别使用绿色/红色前景和不同的淡色背景。当前切片尚不
包含 Mermaid 和内嵌媒体。

Composer 下方的一行紧凑无冗余字段名状态投影，左侧显示模型、强度和模式，右侧显示上下文
占用与工作目录；窄布局会省略过长值。永久快捷键栏已移除，可通过 `/help` 或 F1
查看既有命令参考。等待模型输出时，用户提供的七格折叠脉冲会在待完成助手文案前动画。
...
真实分母。
请求等级与当前实际策略不同时会同时显示两者，例如 `ultracode → xhigh`。状态文案会随
界面语言切换。
