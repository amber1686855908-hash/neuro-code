...
- `SessionStore`：追加带版本事件、保留有序 `SessionItem`，并向代理运行时提供普通消息投影。
...
适配器只会在同一条 assistant 消息包含工具调用时回传该字段；没有工具调用的推理绝不
回传给供应商。
...
顺序。代理运行时只消费普通消息投影；恢复导入会话时，存储只允许在原前缀后追加，
并拒绝改写已保存的上下文。JSON 导出格式版本 2 同时包含两个投影。供应商适配器会
校验图片引用，并且只在协议角色和 URI 形式受支持时使用原生多模态内容块；其他图片
无需执行适配器侧媒体 I/O，直接降级为可见占位文本。保留的上下文项仍不会发送。详见
[ADR 0004](adr/0004-ordered-session-items.md) 和
[ADR 0005](adr/0005-provider-native-image-replay.md)。新生成的思考模式工具轮次改走带类型
消息路径；详见 [ADR 0006](adr/0006-thinking-tool-continuity.md)。
