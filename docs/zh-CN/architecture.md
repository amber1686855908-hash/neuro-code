...
应用自有 TUI 文案通过 `UiLanguage` 选择。注入的 `UiPreferencesStore` 端口只持久化该
表现偏好；JSON 适配器使用与供应商配置分离的用户状态文件并原子写入。英语和简体中文
目录必须具有相同键集合。切换语言会重新渲染界面外壳和可翻译的本地历史，但用户、模型
及工具载荷保持原始表现内容，绝不会送入翻译器。
