...
- Rust 源代码判定基线为 [`xai-org/grok-build`](https://github.com/xai-org/grok-build)
  的提交 `c68e39f60462f28d9be5e683d9cbe2c57b1a5027`。
- 本地源代码检出和其中的 `.ua` 目录只能读取，不能修改。
- 源代码、测试和可执行行为优先于自动生成的 `.ua` 文本。
- 不得机械翻译 crate，也不得按照文件路径机械同步。

如需执行可选的基线校验，请把 `PYGROK_SOURCE_REPOSITORY` 指向本地源码检出，或者向
`scripts/check_source_baseline.py` 传入 `--source`。

## 架构规则

- 通过 `src/pygrok_build/ports` 中的端口交付纵向用户能力。
