...
- 每个持久化回合获得唯一的不透明 `turn_id`。第一次 Provider 请求或工具 body 之前，
  ...
  对计划执行而言，这个接受写入先于新任务创建或排队任务激活，因此进程若在两次操作
  之间退出，会留下明确的未决 attempt，而不是孤立的 `RUNNING` 任务。
...
- CLI、TUI 和 ACP 私有 `neuro-code/session/recovery` 扩展共享应用层恢复服务。它们提供
  有界证据、明确的 `abandon`，以及只针对“无输出且输入精确可重建”的用户回合的明确
  `retry`。重试会放弃旧 attempt 并创建新的回合身份，不会原地续接旧 attempt。
...
| attempt 未终态、输入精确可重建、没有输出、没有工具开始且无事实冲突；请求可以已经标记为 started | `SAFELY_RETRYABLE` | 否；只能明确 retry |
...
存储端口拥有 `start_turn_attempt`、原子恢复事实追加、完成、失败/取消以及明确放弃操作。
SQLite 写锁和事务边界构成持久化边界。如果搜索索引写入失败，整个终态事务回滚，包括
attempt resolution、事件、任务状态和会话项。

## 后果

存在未决 attempt 时，普通 resume 会被阻止。用户必须先查看它，然后明确重试安全的用户回合，
...
聚焦测试覆盖输出前安全分类、request/output/tool write-ahead 事实、完成与失败回滚的原子性、
明确放弃、普通失败过滤、迁移，以及真实子进程退出/重开。持久化输出标记后退出的进程重开
为 `INDETERMINATE`；原子 commit 后退出的进程重开为 `COMMITTED`。
