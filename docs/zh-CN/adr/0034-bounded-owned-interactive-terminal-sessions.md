...
固定 Rust 基线的 `crates/codegen/ptyctl/src/pty.rs` 展示了包含命令、工作目录、环境和尺寸
的终端配置，以及显式读取、写入、resize、等待和终止操作。其
`crates/codegen/xai-grok-pager-pty-harness/src/pty.rs` 还区分了限时读取与输出关闭，处理
原始按键输入和 resize，并在析构时终止受控子进程。这些是行为证据，不是需要翻译的
crate 图。
...
- Linux 测试执行真实 PTY 输入、resize、信号和非零退出；可移植 fake/ctypes 契约覆盖
  Windows 回调、Job 所有权、属性与清理。Windows 原生行为仍需 Windows CI 证据。
