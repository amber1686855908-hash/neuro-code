...
具备各自的原生图片投影。不过 ACP 提示转换此前只接受 Text 和 ResourceLink，ACP 客户端
无法把截图送入这条既有的安全路径。若在 `session/load` 中回放原始媒体，又会把可见历史 API
变成无界的二进制和 URL 泄露通道。
