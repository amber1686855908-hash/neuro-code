...
- 状态：W4 实施中；Gate 1 受限 ConPTY 纵向切片
- 日期：2026-08-15
...
attestation，以及 malformed resize 的有界清理。Python、Git、Node、NUL、curl、应用层
terminal 路由和 W5 developer-tool compatibility 不属于本 ADR 范围。
...
- 只有 Job-owned scope 为空后才执行 `ClosePseudoConsole`；不引入第二套 lifecycle 或
  Job authority。
- 未来 production terminal route 必须先通过 Gate 1 evidence。本 ADR 尚未认证该公开
  route。
