...
- Every persisted turn receives a unique opaque `turn_id`. A turn attempt is
  ...
  For plan execution, this acceptance is persisted before a new task is
  created or a queued task is activated, so a process exit between those
  operations leaves an explicit unresolved attempt instead of an orphaned
  `RUNNING` task.
...
- CLI, TUI, and the ACP private
  ...
  service. They expose bounded evidence, explicit `abandon`, and explicit
  `retry` only for a pre-output user turn with exact input. A retry abandons
  the old attempt and starts a new turn identity; it never resumes the old
  attempt in place.
...
| Open attempt, exact input, no output, no tool start, and no fact conflict; a request may have been marked started | `SAFELY_RETRYABLE` | No; explicit retry only |
...
The storage port owns `start_turn_attempt`, atomic recovery-fact append,
completion, failure/cancellation, and explicit abandonment. SQLite write-lock
and transaction boundaries are the durability boundary. A failed index/search
write rolls the entire terminalization back, including the attempt resolution,
event, task state, and session items.

## Consequences

Normal resume is blocked while an unresolved attempt exists. A user must first
inspect it and either explicitly retry a safe user turn or abandon it. An
indeterminate attempt can be audited and abandoned, but this phase does not
...
write-ahead facts, atomic completion and failure rollback, explicit abandon,
normal failure filtering, migration, and real child-process exit/reopen. A
process exit after a durable output marker reopens as `INDETERMINATE`; a
process exit after the atomic commit reopens as `COMMITTED`.
