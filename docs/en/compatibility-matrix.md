...
| OpenAI-compatible/xAI model endpoint | M2 | partial | Custom model configuration; Chat Completions adapter first |
| Anthropic provider | M2 | partial | Native Messages SSE, text/reasoning, tools, usage, errors, and secret redaction implemented; live/model-specific fixtures pending |
| Gemini provider | M2 | partial | Native `streamGenerateContent` SSE, text/thoughts, tools, usage, errors, and thought-signature round trip implemented; live/model-specific fixtures pending |
