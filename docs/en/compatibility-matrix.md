...
| Named provider profiles and selection | M2 | partial | Four explicit wire protocols, default/one-shot selection, legacy TOML compatibility, and redacted `providers list/inspect` implemented; interactive selector intentionally deferred |
...
| SQLite session event store | M2 | partial | Schema v2, transactional v1 migration, profile-affinity metadata, canonical ordered session-item persistence, message projection, append-only prefix checks, list, resume, and export implemented |
...
| Full-screen/minimal TUI | M3 | planned | Independent terminal UI implementation |
